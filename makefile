...
lintArgs += --line-length=100 --dupl-threshold=50 --cyclo-over=10
...
$(buildDir)/coverage.%.out:$(testRunDeps)
	$(vendorGopath) go test --timeout=1m -covermode=count -coverprofile=$(buildDir)/coverage.$(subst /,-,$*).out $(projectPath)/$(subst -,/,$*)
